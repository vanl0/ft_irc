...
Server::Server(int const port): _port(port), _running(true){
        ...
        for (size_t i = 0; i < _fds.size(); ++i) {
            if (_fds[i].revents & POLLIN) { //if flag activated for that socket
                if (_fds[i].fd == _server_fd) { //if revents flag is from the server means is a new client
                    ...
                    ssize_t bytes_read = recv(_fds[i].fd, _buffer, sizeof(_buffer) - 1, 0);//read the message from the fd
                    if (bytes_read < 0) {
                        perror("recv");
                        close(_fds[i].fd);
                        ...
                        std::cout << "Received from fd " << _fds[i].fd << ": " << _buffer << std::endl;
                    }
                }
            }
        }
